...
fn process_file(path: &str, max_minutes : u32) -> Result<u32, GenError> {
    ...
    let ref mut current_playfield = &mut playfield;
    let ref mut next_playfield = &mut work_buffer;
    ...
    while minute <= max_minutes {
        ...
        println!("After {} minutes value is {} ", minute, find_value(&current_playfield, rows, cols));
