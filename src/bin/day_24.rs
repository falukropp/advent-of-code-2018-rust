...
impl Group {
    ...
    fn damage_dealt(&self, other: &Group) -> i32 {
        ...
        let base_damage = self.attack_power * self.size;
...
fn play_game(groups: &mut Vec<Group>) -> i32 {
    ...
    loop {
        ...
        // Targeting phase
        // --------------------
        let mut targetting = vec![vec![std::u32::MAX; groups.len()]; 2];
        // targetting[0] == Immune
        ...
        for source_idx in 0..groups.len() {
            ...
            for target_idx in 0..groups.len() {
                ...
                // else if damage_dealt == max_damage_dealt {
                //     if target_group.effective_power()
                //         > groups[selected_target_idx].effective_power()
                //     {
                //         selected_target_idx = target_idx;
                //     } else if target_group.effective_power()
                //         == groups[selected_target_idx].effective_power()
                //     {
                //         if target_group.initiative > groups[selected_target_idx].initiative {
                //             selected_target_idx = target_idx;
                //         }
                //     }
                // }
